...
///
/// - For `#[repr(C)]` field-less enums, [the underlying representation is platform-dependent][repr-c-enum].
///   In this case, the macro requires explicit types to be specified via `#[into(...)]` to
///   ensure the conversion is predictable across different architectures.
///
/// [repr-c-enum]: https://doc.rust-lang.org/reference/type-layout.html#r-layout.repr.c.enum
///
...
/// ```compile_fail
/// # use kernel::macros::Into;
/// // `#[repr(C)]` field-less enums without `#[into(...)]` are not allowed.
/// #[derive(Into)]
/// #[repr(C)]
/// enum Foo {
///     A,
...
///
/// - For `#[repr(C)]` field-less enums, [the underlying representation is platform-dependent][repr-c-enum].
///   In this case, the macro requires explicit types to be specified via `#[try_from(...)]`
///   to ensure the conversion is predictable across different architectures.
///
...
/// ```compile_fail
/// # use kernel::macros::TryFrom;
/// // `#[repr(C)]` field-less enums without `#[try_from(...)]` are not allowed.
/// #[derive(TryFrom)]
/// #[repr(C)]
/// enum Foo {
///     A,
