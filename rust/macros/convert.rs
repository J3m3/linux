...
fn derive(target: DeriveTarget, input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    let (helper_tys, repr_ty, repr_c) = parse_attrs(target, &input.attrs)?;
    ...
    if let Some(repr_c) = repr_c {
        if helper_tys.is_empty() {
            let msg = format!(
                "field-less `repr(C)` enum size is platform-dependent; \
                use `#[{}(...)]` to specify a fixed size.\n\
                See: https://doc.rust-lang.org/reference/type-layout.html#r-layout.repr.c.enum",
                target.get_helper_name()
            );
            combine_error(syn::Error::new(repr_c.span(), msg));
        }
    }
...
fn parse_attrs(
    target: DeriveTarget,
    attrs: &[Attribute],
) -> syn::Result<(Vec<ValidTy>, Ident, Option<Ident>)> {
    ...
    for attr in attrs {
        if attr.path().is_ident("repr") {
            attr.parse_nested_meta(|meta| {
                ...
                if ident.as_ref().is_some_and(|ident| *ident == "C") {
                    repr_c = ident.cloned();
                } else {
                    repr_ty = ident.filter(|ident| is_valid_primitive(ident)).cloned();
                }
...
fn derive_for_enum(
    ...
    let target_tys = if helper_tys.is_empty() {
        &[ValidTy::Primitive(repr_ty.clone())]
    } else {
        helper_tys
    };
            ...
            const _: () = {
                const fn fits(d: #repr_ty) -> ::core::primitive::bool {
                    ...
                    if is_src_signed && is_dst_signed {
                        ...
                        // overflow since since `u128::MAX` is greater than `i128::MAX`.
...
impl Bounded {
    const NAME: &str = "Bounded";
    const QUALIFIED_NAME: &str = "::kernel::num::Bounded";
