...
fn derive(target: DeriveTarget, input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    let Some(repr_ty) = repr_ty else {
        let msg = format!(
            "`#[derive({})]` requires explicit discriminant representation; \
            consider specifying `#[repr(i32)]`, `#[repr(u32)]`, etc.",
            target.get_trait_name(),
        );
        return Err(syn::Error::new(data_enum.enum_token.span(), msg));
    };
...
fn parse_attrs(
    target: DeriveTarget,
    attrs: &[Attribute],
) -> syn::Result<(Vec<Type>, Option<Ident>)> {
